    ...
    <div v-if="selectedCard !== null" class="flex flex-col items-center justify-center">
      <div class="relative">
        <div class="w-64 h-96 text-sm overflow-hidden rounded-xl cursor-pointer" @click="handleBackClick">
          <uno-card
            :value="selectedCard"
            :color="getCardColor(DEFAULT_CARDS.indexOf(selectedCard))"
            :flipped="isRevealed"
          >
            >
            <uno-card-chameleon :value="selectedCard" :color="getCardColor(DEFAULT_CARDS.indexOf(selectedCard))" />
          </uno-card>
        </div>
        <div
          v-if="!isRevealed"
...
const DEFAULT_CARDS = props.cards;
